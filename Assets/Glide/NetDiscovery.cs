...
public class NetDiscovery : MonoBehaviour
{
	...
	public UnityEngine.Events.UnityEvent<DiscoveryInfo> onReceivedServerResponse = new();
	...
	public static NetDiscovery Instance { get; private set; }
	...
	public static byte[] GetDiscoveryRequestData()
	{
		...
		var dict = new Dictionary<string, string>(System.StringComparer.InvariantCulture)
			{ { kSignatureKey, GetCachedSignature() } };
	...
	static IPAddress[] GetBroadcastAdressesFromNetworkInterfaces()
	{
		...
		var nifs = NetworkInterface.GetAllNetworkInterfaces()
			...
			.Where(nif => nif.NetworkInterfaceType == NetworkInterfaceType.Wireless80211 ||
	...
	public static bool IsDataFromServerValid(DiscoveryInfo data)
	{
		...
		return data.KeyValuePairs.ContainsKey(kSignatureKey) && data.KeyValuePairs[kSignatureKey] ==
		                                                     GetCachedSignature()
